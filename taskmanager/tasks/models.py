...
class Task(models.Model):
    ...
    status = models.CharField(max_length=20, choices=TASK_STATUS_CHOICES, default="UNASSIGNED", db_comment="Can be UNASSIGNED, IN PROGRESS, DONE, or ARCHIVED")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    creator = models.ForeignKey(User, on_delete=models.CASCADE, related_name="created_tasks")
    owner = models.ForeignKey(User, on_delete=models.SET_NULL, related_name="owned_tasks", blank=True, null=True)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(check=models.Q(status__in=[s[0] for s in TASK_STATUS_CHOICES]), name="valid_status"),
            models.CheckConstraint(check=models.Q(due_date__gt=models.F("created_at")), name="due_date_after_created_at"),
        ]
